...
import { useOrderForm } from '../hooks/useOrderCalculations';


export const OrderForm = () => {
  const {
    stockInfo, orderMode, exchange, quantity, orderType, productType,
    availableMargin, requiredMargin, isMarginSufficient, effectivePrice,
    setOrderMode, setExchange, setQuantity, setPrice, setOrderType, setProductType, resetForm
  } = useOrderForm();

  const handleAction = () => {
    if (!isMarginSufficient) return;
    alert(`Placed ${orderMode} order: ${quantity} ${stockInfo.name} @ ${effectivePrice}`);
  };

  return (
    <div className="w-125 bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden mx-auto">
      <div className="bg-[#EEF2F6] px-5 py-4 border-b border-gray-200 flex flex-col gap-3">
        <div className="flex items-center justify-between">
          ...
          <TabBarComponent
            ...
            value={orderMode}
            onChange={(val) => setOrderMode(val as any)}
          />
        </div>
        <RadioToggle
          options={[
          ...
          value={exchange}
          onChange={(val) => setExchange(val as any)}
        />
      </div>

      <div className="p-6 flex flex-col gap-7">
        <div className="grid grid-cols-2 gap-x-8 gap-y-6">
          <StepperInput label="Quantity" value={quantity} onChange={setQuantity} min={1} />
        ...
        <div className="flex items-end justify-between pt-4 mt-2">
          <div className="flex gap-8">
            <div className="flex flex-col">
              ...
              <span className={`text-sm font-semibold ${!isMarginSufficient ? 'text-red-600' : 'text-gray-900'}`}>
          ...
          <div className="flex items-center gap-4">
            <Button variant="ghost" className="text-blue-600 font-semibold px-3" onClick={resetForm}>
              Cancel
            </Button>
            <Button
              variant={orderMode === 'Buy' ? 'buy' : 'sell'}
              className="w-28 font-semibold shadow-md"
